...
install() {
    rm_dir=${DESTDIR}${LIBDIR}/${CAMLP5N} # variant: `${DESTDIR}${LIBDIR}/${CAMLP5N}` is extracted from the orignal `rm -rf "${DESTDIR}${LIBDIR}/${CAMLP5N}"` as `rm_dir`.
    rm -rf "${rm_dir}" # bug here: will delete even if variables not set
