...
# -----
# diff: create an indirect variable definition that shellcheck doesn't understand
mydef() {
        echo "$2" > tmp
        read "$1" < tmp
        rm tmp
...
install_nvm_as_script() {
  ...
  mydef _source "$local_NVM_SOURCE" # diff: define $_source
  nvm_download -s "$_source" -o "$NVM_DIR/nvm.sh" || { # NO bug here: _source is set
    echo >&2 "Failed to download '$_source'.." # NO bug here: _source is set
    unset local_NVM_SOURCE
    return 1
  }
  unset local_NVM_SOURCE
