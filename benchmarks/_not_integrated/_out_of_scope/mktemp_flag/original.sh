...
tmpdir() {
	if [[ -d /dev/shm && -w /dev/shm && -x /dev/shm ]]; then
		tmp_dir="$(TMPDIR=/dev/shm mktemp -t "$template" -d)" # bug here: -d must come before -t
	else
...
case "$command" in
	...
	edit)
		...
		tmp_file="$(TMPDIR="$tmp_dir" mktemp -t "$template" -d)" # bug here: $tmp_dir will be a file, not a directory
