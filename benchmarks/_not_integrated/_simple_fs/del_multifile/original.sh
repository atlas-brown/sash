#!/usr/bin/env bash
# https://github.com/JetBrains/JetBrainsMono/commit/f011e255926c684fe780cb86be2dff6c905d7359
...
function cleanup() {
  for delme in "$@"; do
    unlink "${delme}" || die "Unable to unlink: ${delme}" # bug here:
  done
}
...
function main() {
  ...
  echo "Cleaning up..."
  cleanup "${TEMP_LATEST_INFO} ${TEMP_FONTS_ARCHIVE}" # bug here: arguments are not split
