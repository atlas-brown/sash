...
am_doing_phase () {
	...
	local_x=""
	for local_x in "$@"; do
		if echo " $WHAT_TO_DO " | grep -q " $local_x "; then return 0; fi
	done
	return 1
}
...
if [ $# != 0 ] ; then
	...
	case "$1" in
		...
		if [ "$1" = "--make-tarball" -a -n "$2" ] ; then
			MAKE_TARBALL="$2"
			shift 2
		elif [ "$1" != "${1#--mnake-tarball=}" ]; then # bug here: mnake instead of make (the prefix of $1 will never match)
			MAKE_TARBALL="${1#--mnake-tarball=}" # bug here: mnake instead of make (the prefix of $1 will never match)
			shift
		else
			error 1 NEEDARG "option requires an argument %s" "$1"
		fi
