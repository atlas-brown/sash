    ...
    then
        ...
        "gzip"|"gz"|"bzip"|"bzip2")
            ...
            if [ -n "$compress_bin" ] && [ -x "$compress_bin" ]; then
                ...
                exitcode=$? # diff: instead of checking exit code directly, assign to variable first
...
__get_flags_relative_blacklist()
{
    switch="$1"
    target="$2"
...
__make_remote_tarball_archives()
{
    ...
    for target in $BM_TARBALL_TARGETS # this isn't defined locally anyways
    do
        if [ -z "$target" ]; then
            continue
        fi
