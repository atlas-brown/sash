...
curl -s https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/misc/build.func | . /dev/stdin
...
echo "${CREATING}${GN}${APP} setup has been successfully initialized!${CL}"
echo "${INFO}${YW} Access it using the following URL:${CL}"
echo "${TAB}${GATEWAY}${BGN}http://${IP}:5006${CL}"
