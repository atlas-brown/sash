...
#globals
whitelist=/etc/pihole/whitelist.txt # bug here (start): whitelist is assigned here as a path
...
PopWhitelistFile(){
	...
	if [ ! -f "$whitelist" ];then # bug here (cont'd): whitelist is checked for existence or created here
  	  touch $whitelist
	fi
	for dom in ${domList}
	do
...
#| sed 's/\./\\./g'
	...
	if $bool; then
	  ...
	  if $versbose; then
		echo "::: Adding $1 to whitelist.txt..."
	  fi
	  echo "$1" >> $whitelist # bug here (cont'd): whitelist is written to here
		modifyHost=true
		if $versbose; then
	  	echo " done!"
	  fi
...
RemoveDomain(){
  ...
  bool=false
  grep -Ex -q "$1" $whitelist || bool=true # bug here (cont'd): whitelist is read here
  if $bool; then
...
ModifyHostFile(){
	 if $addmode; then
	    ...
	    if [ -r "$whitelist" ];then # bug here (cont'd): whitelist is confirmed to be readable here
        # Remove whitelist entries
				numberOf=$($whitelist | sed '/^\s*$/d' | wc -l) # bug here (finally): should be 'cat $whitelist | sed ...'
        plural=; [ "$numberOf" != "1" ] && plural=s
        echo ":::"
        echo "::: Modifying HOSTS file to whitelist $numberOf domain${plural}..."
	    ...
	    do
	    	if [ -n "$piholeIPv6" ];then
	    	  echo ":::    Un-whitelisting $rdom on IPv4 and IPv6..."
	    	  echo "$rdom" | awk -v ipv4addr="$piholeIP" -v ipv6addr="$piholeIPv6" '{sub(/\r$/,""); print ipv4addr" "$0"\n"ipv6addr" "$0}' >> $adList
	    	  echo " done!"
	      else
	        echo ":::    Un-whitelisting $rdom on IPv4"
	      	echo "$rdom" | awk -v ipv4addr="$piholeIP" '{sub(/\r$/,""); print ipv4addr" "$0}' >>$adList
	      	echo " done!"
	      fi
	      echo ":::        Removing $rdom from whitelist.txt..."
