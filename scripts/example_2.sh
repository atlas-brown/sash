#! /usr/bin/env sh

download_repos() {
  user_name="$1"

  temp_dir=$(mktemp -q /tmp/repo_archive.XXXXXX)  # Hint: what does the -d flag of this command do?
  if [ $? -ne 0 ]; then  # $? is the (numeric) exit code of the previous command
      echo "$0: Can't create a temp file!"
      exit 1
  fi

  echo "Using the following temp dir: $temp_dir"

  cd "$temp_dir" && do_some_stuff

  cd ~  # ~ expands to $HOME
  destination="${1}_repo_archive.tar"
  tar -cvf "$destination" "$temp_dir"

  # Cleanup
  rm -rf "$temp_dir"

  echo "All repositories of $user_name have been written to $destination"
}

main() {
    if [ "$#" -ne 1 ]; then
        echo "Usage: download_all_github_repos.sh [github_username]"
        exit 1
    fi

    download_repos "$1"
}

main "$@"
